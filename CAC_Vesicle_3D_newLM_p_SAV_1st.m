function [phi, eta, lambda] = CAC_Vesicle_3D_newLM_p_SAV_1st(pde,domain,Nx,Ny,Nz,time,option)
...
% Initial energy
if 1 == option.energyflag
    calculate_energy1(out1,out2,t,phi0);
end
...
%         rhs = rhs./M;
    ...
    %step 2
    lambda_initial = fun_inner(delta_B(phi_star),phi0-phi_1-phi_2) ./ fun_inner(delta_B(phi_star),phi_3);
    ...
    %step 3
    eta_initial = eta0;
    eta = fsolve(@(eta)non_fun2(eta,lambda,phi_1,phi_2,phi_3,phi0),eta_initial,...
                   opts);
%     eta = 1;
...
function [] = calculate_energy1(out1,out2,t,phi)
global epsilon
...
energy_modified = energy_linear + energy_nonlinear;
...
function r = non_fun2(eta,lambda,phi_1,phi_2,phi_3,phi0)
global hx hy hz
    left = fftn(fun_W(phi_1+eta.*phi_2+lambda.*phi_3)-fun_W(phi0));
    right1 = eta.*fftn(delta_W(phi0).*(phi_1+eta.*phi_2+lambda.*phi_3-phi0));
    right2 = lambda.*fftn(delta_B(phi0).*(phi_1+eta.*phi_2+lambda.*phi_3-phi0));
    r = left(1,1,1)*hx*hy*hz - right1(1,1,1)*hx*hy*hz  + right2(1,1,1)*hx*hy*hz ;
end
