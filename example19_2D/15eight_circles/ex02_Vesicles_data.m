...
pde = struct('epsilon',epsilon, ...
    ...
    function z = init(x,y)
        ...
        z =  tanh((rl(1)-sqrt((x-xl(1)).^2./rx.^2+(y-yl(1)).^2./ry.^2))./(sqrt(2)*epsilon)) ...
            ...
            +tanh((rl(6)-sqrt((x-xl(6)).^2./rx.^2+(y-yl(6)).^2./ry.^2))./(sqrt(2)*epsilon)) ...
            +tanh((rl(7)-sqrt((x-xl(7)).^2./rx.^2+(y-yl(7)).^2./ry.^2))./(sqrt(2)*epsilon)) ...
            +tanh((rl(8)-sqrt((x-xl(8)).^2./rx.^2+(y-yl(8)).^2./ry.^2))./(sqrt(2)*epsilon))+7;
    end
