...
pde = struct('epsilon',epsilon, ...
    ...
    function z = init(x,y,z)
        d = 0.28*pi;
        rl =[0.2*pi,0.2*pi,0.2*pi,0.2*pi,0.2*pi,0.2*pi];
        xl = [pi,pi-d,pi+d,pi-2*d,pi,pi+2*d];
        yl = [pi,pi,pi,pi,pi,pi];
        zl = [pi+2*d,pi,pi,pi-2*d,pi-2*d,pi-2*d];
        rx = 1;
        ry = 1;
        rz = 1;
        ...
        z =  tanh((rl(1)-sqrt((x-xl(1)).^2./rx.^2+(y-yl(1)).^2./ry.^2+(z-zl(1)).^2./rz.^2))./(sqrt(2)*epsilon)) ...
            ...
            +tanh((rl(6)-sqrt((x-xl(6)).^2./rx.^2+(y-yl(6)).^2./ry.^2+(z-zl(6)).^2./rz.^2))./(sqrt(2)*epsilon))+5;
    end
