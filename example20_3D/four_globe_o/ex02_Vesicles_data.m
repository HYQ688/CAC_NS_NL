...
pde = struct('epsilon',epsilon, ...
    ...
    function z = init(x,y,z)
        d = 0.28*pi;
        rl =[0.28*pi,0.28*pi,0.28*pi,0.28*pi];
        xl = [pi,pi-sqrt(2)*d,pi+sqrt(2)*d,pi];
        yl = [pi,pi+d,pi+d,pi-sqrt(2)*d];
        zl = [pi+sqrt(3)*d,pi,pi,pi];
        rx = 1;
        ry = 1;
        rz = 1;
        ...
        z =  tanh((rl(1)-sqrt((x-xl(1)).^2./rx.^2+(y-yl(1)).^2./ry.^2+(z-zl(1)).^2./rz.^2))./(sqrt(2)*epsilon)) ...
            ...
            +tanh((rl(4)-sqrt((x-xl(4)).^2./rx.^2+(y-yl(4)).^2./ry.^2+(z-zl(4)).^2./rz.^2))./(sqrt(2)*epsilon)) +3;
    end
