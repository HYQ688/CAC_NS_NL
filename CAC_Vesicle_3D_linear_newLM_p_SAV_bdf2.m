function [phi, U, lambda] = CAC_Vesicle_3D_linear_newLM_p_SAV_bdf2(pde,domain,Nx,Ny,Nz,time,option)
...
[phi1,U1,~] = CAC_Vesicle_3D_linear_newLM_p_SAV_1st(pde,domain,Nx,Ny,Nz,time1,option);
...
%         tmp  = ephi_t./M - lap_diff(emu);
    ...
    lambda = fun_inner(delta_B(phi_star),4*phi1-phi0-3*phi_1) ./ fun_inner(delta_B(phi_star),3*phi_2);
...
%     lambda = 0;
    ...
    if 1 == option.energyflag
        calculate_energy1(out1,out2,t,phi1,phi0,U1,U0);
    end
...
global dt k2 M epsilon Lx Ly Lz S1 S2 S3
    ...
    r(1,1,1) = phihat(1,1,1)./(3/2/dt/M + L1(1,1,1)+ L2(1,1,1) - 1/(Lx*Ly*Lz).*L1(1,1,1).*Lx.*Ly.*Lz- 1/(Lx*Ly*Lz).*L2(1,1,1).*Lx.*Ly.*Lz);
    result = real(ifftn(r));
end
...
function [] = calculate_energy1(out1,out2,t,phi1,phi0,U1,U0)
...
energy_part2 = 1/2*(U1.^2 + U_star.^2) - C0;

energy_original = energy_linear + energy_nonlinear;

energy_modified = energy_part1 + energy_part2+ energy_S;
